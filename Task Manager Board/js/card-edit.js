...
cardEdit.submit = function (evt) {
	...
	if (title) {
		...
		cardEdit.card.createdAt = new Date(createdAt).toDateString()
		cardEdit.card.status = status
		cardEdit.card.surroundStrongTag.innerText = title;
		...
		cardEdit.card.createdAtNode.innerText = new Date(createdAt).toDateString();
